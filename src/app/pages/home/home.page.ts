...
export class HomePage implements OnInit {
  ...
  loadMap() {
    ...
    markerCluster.on(GoogleMapsEvent.MARKER_CLICK).subscribe((params) => {
      ...
      this.cardShown = true;
      this.name = marker.get("name")
      this.address = marker.get("address")
      console.log(this.name)
    })
    // })

    this.map.on(GoogleMapsEvent.MAP_CLICK).subscribe( () =>{
      if(this.cardShown) this.cardShown = false;
    })
    ...
    return [
      {
        ...
        "phone": "808-484-0000",
        "icon": "./assets/img/parking-32.png"
      },
      {
        "position": {
          "lat": 21.3871,
